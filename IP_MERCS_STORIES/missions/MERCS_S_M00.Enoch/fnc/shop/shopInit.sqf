private ["_getClasses", "_setPrices", "_isFromExcludedMod", "_autoIndexing", "_defaultNavigation", "_inPossession", "_uniformsInPossession", "_enhancementsInPossession", "_vehiclesInPossession", "_serverSide", "_box", "_missions", "_excludedMods", "_personnelCategories", "_weaponCategories", "_magazines", "_itemCategories", "_uniformCategories", "_enhancements", "_enhancementClasses", "_vehicleCategories"];
_getClasses = {
	private "_arr";
	_arr = [];
...
_isFromExcludedMod = {
	params ["_excludedMods", "_className", "_isExcluded"];	
	_isExcluded = false;
	
	{
		{
			if (toLower(_className) == toLower(_x)) exitWith {
				systemChat (_className + " == " + _x);
				_isExcluded = true;
			};
		} forEach (getArray(configFile >> "CfgPatches" >> _x >> "weapons") + getArray(configFile >> "CfgPatches" >> _x >> "units"));
	} forEach _excludedMods;
...
_excludedMods = getArray(missionConfigFile >> "ShopMetaInformation" >> "excludedMods");
...
if (_autoIndexing) then {
	...
	{
		if ((([_x] call BIS_fnc_baseWeapon) == _x) && {([(missionConfigFile >> "ShopWeapons"), _x] call IP_fnc_getConfigCategory) == ""} && {!([_excludedMods, _x] call _isFromExcludedMod)}) then {
			private "_category";
...
IP_AvailableItems = [];
{
	...
	{
		if (((isNumber(configFile >> "CfgWeapons" >> _x >> "scope")) && {getNumber(configFile >> "CfgWeapons" >> _x >> "scope") == 2})
			OR {(isNumber(configFile >> "CfgVehicles" >> _x >> "scope")) && {getNumber(configFile >> "CfgVehicles" >> _x >> "scope") == 2}} 
			OR {(isNumber(configFile >> "CfgGlasses" >> _x >> "scope")) && {getNumber(configFile >> "CfgGlasses" >> _x >> "scope") == 2}}) then {
				_items pushBack _x;
		};
...
if (_autoIndexing) then {
	...
	
	{
		...
		if (_backpackCheck && {([(missionConfigFile >> "ShopItems"), _x] call IP_fnc_getConfigCategory) == ""} && {!([_excludedMods, _x] call _isFromExcludedMod)}) then {			
