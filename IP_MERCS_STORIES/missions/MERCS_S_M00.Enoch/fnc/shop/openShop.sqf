...
switch (_category) do {
	...
	case "Weapons": {
		...
		_weapons = _box call BIS_fnc_getVirtualWeaponCargo;
	...
	case "Items": {
		...
		_items = ((_box call BIS_fnc_getVirtualItemCargo) - (player getVariable ["IP_ShopUniforms", []])) + (_box call BIS_fnc_getVirtualBackpackCargo);
		for "_i" from 0 to ((count _items) - 1) do {
			_cell = _items select _i;
			//_entry = _cell select 0;
			//_count = _cell select 1;
			_parent = _cell call IP_fnc_getTopConfig;
			_title = getText (configFile >> _parent >> _cell >> "displayName");
			_picture = getText(configFile >> _parent >> _cell >> "picture");			
			_text = /*str(_count) + " " + */ _title;
			_index = lbAdd [1501, _text];
			lbSetPicture [1501, _index, _picture];
			lbSetData [1501, _index, _cell];
		};
	...
	case "StockMarket": {
		...
		for "_i" from 0 to ((count IP_AvailableStocks) - 1) do {
			...
			_index = lbAdd [1500, _text];
			lbSetPicture [1500, _index, ("IP_CMP_ICE\IP_CMP_ICE\img\shop\" + _picture + ".paa")];
		};
