...
#define WAIT waitUntil {_speaker kbWasSaid [_receiver, _topic, _sentence, 60]};
...
if !(player diarySubjectExists "ConvLog") then {
	player createDiarySubject ["ConvLog", "Conversation Log"];
};

if !(_speaker kbHasTopic _topic) then {
...
if (_subs) then {
	...
	if (_liveFeed && {(vehicle _speaker) == _speaker} && {!visibleMap}) then {
		...
		_picture = _speaker getVariable ["IP_Avatar", "Campaigns\IP_CMP_ICE\img\defaultAvatar.jpg"];
